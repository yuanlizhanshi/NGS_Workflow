...
def usage():
    print('Usage: python script.py [gtf_file] [genome_file] [subset_file]')
...
if __name__ == '__main__':
    try:
        ...
        subset_file = pd.read_csv(sys.argv[3],sep='\t')
        transcript_id = subset_file.transcript_id.tolist()
        new_id = subset_file.seq_id.tolist()
        py_extractSequence(gtf_file,genome_file,transcript_id,new_id,type = 'exon',out_file = 'longest_transcript.fasta')
    except IndexError:
        usage()
